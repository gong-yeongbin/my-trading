...
my-trading/
  ...
  internal/ls/                LS증권 웹소켓 클라이언트: 토큰, 실시간 뉴스·지수 구독, 재연결
...
- TUI 로그 패널은 시작 시 마지막 200줄을 읽고, 1초마다 파일 크기를 확인해 늘어난 만큼 읽는다.
- 로테이션은 하지 않는다 (1차).
...
- 모든 서브커맨드와 TUI는 10.3절의 로그 파일에 기록한다. 파일을 열 수 없으면 stderr로 대신 쓰고 계속 진행한다.
