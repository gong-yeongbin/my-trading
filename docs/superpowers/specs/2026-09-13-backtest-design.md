...
`fetch`와 `universe`만 한투 데이터 API를 쓴다. `watch`는 네트워크를 쓰지 않는다. TUI는 SQLite를 읽고, 실시간 뉴스·지수는 LS 웹소켓으로, 보유 종목은 한투 잔고 조회로 받는다. TUI는 보기 전용이라 수집을 실행하지 않는다 (수집은 새벽에 `fetch`를 스케줄러로 돌린다).
...
| 왼쪽 메뉴 | 관심종목 / 보유종목 / 로그 | | |
| 가운데 | 선택한 패널 (아래) | | |
| 하단 | `[장 상태]` + 코스피·코스닥 현재 지수와 등락률. 오른쪽 끝 키 안내 | `market`, `ls` (`IJ_`, `JIF`) | 장 상태는 시계 기준(`market.Status`: 휴장/장전/장중/동시호가/장마감/시간외), 그날 JIF 를 받았으면 그 값. 지수는 수신 즉시. 미연결이면 `미연결`, 연결됐지만 값이 없으면 `연결됨 · 장외`. 7단계(SQLite 지수 봉)부터는 전일 종가와 `(전일)` 표시 |
