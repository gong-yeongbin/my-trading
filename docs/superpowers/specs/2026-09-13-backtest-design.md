...
universe:
  ...
  group_codes: [ST]            # 보통주만
  exclude_flags: [거래정지, 정리매매, 관리종목, 시장경고, 단기과열, 이상급등, SPAC, 우선주]
...
strategy:
  ...
  min_price: 1000              # 전일 종가가 이 값 미만이면 관심종목 제외
```
...
- 사용하는 필드: `그룹코드`(ST=보통주), `거래정지`, `정리매매`, `관리종목`, `시장경고`(0 아니면 투자주의/경고/위험), `단기과열`, `이상급등`, `SPAC`, `우선주`(0=보통주, 1 구형, 2 신형, 9 기타 — 0 이 아니면 우선주). 코스닥 파일의 대응 컬럼명은 예제 코드를 따른다.
- `universe.group_codes`에 해당하고 `universe.exclude_flags`에 해당하는 플래그가 모두 꺼진 종목만 `symbols` 테이블에 저장한다. 마스터에 있지만 이번에 제외된 종목은 `symbols`에서 지운다(봉은 남긴다 — 플래그가 풀려 다시 들어오면 `fetch`가 빈 구간을 채운다). 마스터에서 사라진 종목은 건드리지 않는다.
...
`AvgTurnover` 내림차순으로 정렬한다 (유동성 큰 순 — 종가 베팅은 체결이 돼야 하므로), 같으면 코드순. 다음은 목록에서 뺀다: 기준일(코스피 지수 마지막 봉 날짜)에 봉이 없는 종목(상장폐지·거래정지·수집 미완), 전일 종가가 `strategy.min_price`(기본 1,000원) 미만인 종목, 시장 필터가 `차단`인 시장의 종목(우선주는 유니버스에서 이미 빠진다).

관찰(2026-09-17 실데이터): `MinChangePct` 오름차순이던 초기 정렬에서는 상위가 저유동 우선주로 채워졌다 — 그래서 우선주 제외·거래대금 정렬로 바꿨다. 후보가 1,000개를 넘는 것은 정상이다: 이 표는 장중 판정(나중 단계)의 입력이다.
...
────────────────────────────────────────────────────────────────────────────────────────────────────
  ...
  > 관심종목  │  관심종목  (09-12 기준, 37개)                        코스피 진입가능 · 코스닥 차단
    보유종목  │
    로그      │  종목명            시장     전일종가     필요종가    필요상승     필요거래량  거래대금(억)
              │  ──────────────────────────────────────────────────────────────────────────────
...
| 패널 | 제목 줄 | 표 | 출처 | 갱신 |
|---|---|---|---|---|
| 관심종목 | 기준일, 종목 수, 시장 필터 상태 (전일 지수 종가 vs 20일선 → 시장별 `진입가능` / `차단`) | 종목명, 시장, 전일종가, 필요종가, 필요상승, 필요거래량, 거래대금(억). 평균 거래대금 내림차순, 차단된 시장·1,000원 미만·우선주 제외 | `screener`, SQLite | 시작 시 1회 + 자동 수집 완료 후 재계산 (`screener.Run`) |
| 보유종목 | 종목 수, 평가금액, 손익(금액·%), 현금 | 종목명, 시장, 수량, 매입가, 현재가, 손익, 수익률, 보유일 | `kis` 잔고 조회 (7.6절), `trade_env` 서버·키·계좌 | 켤 때 1회, 매일 장전 08:59에 1회(마지막 조회가 그날 08:59 이전이면 — 잠자기로 지나쳤어도 깨어나면 1회), 보유종목 패널에서 `r` 키로 즉시. 장중 폴링은 없다(주문 기능이 생기면 체결 확인 뒤 갱신). 패널이 활성일 때 하단 키 안내에 `갱신 HH:MM:SS  r 새로고침`. 실패하면 로그만 남기고 마지막 값 유지, 한 번도 성공 못 했으면 `미연결`로 두고 1분마다 재시도. 정렬은 평가금액(현재가×수량) 큰 순. 보유 없으면 `보유 없음` + 현금 |
...
- TUI는 앱키·계좌번호 없이 시작된다. 매매 키나 `KIS_DEMO_ACCOUNT`/`KIS_REAL_ACCOUNT`(`trade_env`에 따라)가 없으면 보유 종목 패널에 `미연결`, LS 앱키가 없거나 웹소켓이 끊기면 뉴스 줄에 `미연결`, 지수 줄에 전일 종가와 `(전일)`을 표시한다. 잔고 조회 실패는 로그에 남기고 다음 조회(장전·`r`)에 다시 시도한다(첫 성공 전이면 1분마다). 어떤 경우에도 TUI는 종료하지 않는다.
